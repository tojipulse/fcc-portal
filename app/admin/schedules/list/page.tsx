...
function buildTextList(groups: DateScheduleGroup[]) {
  return groups
    .map((group) => {
      const scheduleLines = group.schedules.map((schedule) => {
        const subTypeText = schedule.subType
          ? `（${schedule.subType}）`
          : "";

        const endTimeText = schedule.endTime
          ? `-${schedule.endTime}`
          : "";

        return `${schedule.gradeLabel}　${schedule.type}${subTypeText}　${schedule.place}　${schedule.startTime}${endTimeText}`;
      });
        ...
        <Card>
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <button
              ...
              onClick={handleCopy}
              disabled={!textList}
              className="shrink-0 rounded-lg bg-slate-900 px-4 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              {copied ? "✓ コピー済み" : "📋 コピー"}
            </button>
          </div>
          ...
          <textarea
            ...
            readOnly
            aria-label="予定テキスト一覧"
            className="min-h-[240px] w-full resize-none rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold leading-relaxed text-slate-700 outline-none"
          />
        </Card>
            ...
            >
              ...
              <div className="mt-4 space-y-4">
                {group.schedules.map((schedule) => (
                  <Link
                    key={schedule.id}
                    href={`/admin/schedules/edit?id=${schedule.id}`}
                    className="block"
                  >
                    <article className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition active:bg-slate-100">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="pt-1 text-2xl font-black text-slate-400">
                          ＞
                        </div>
                      </div>
                    </article>
